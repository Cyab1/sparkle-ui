...
import {
  ...
  remove,
  push,
  onValue, // ← added for Community.tsx real-time listeners
} from "firebase/database";
import { getStorage } from "firebase/storage"; // ← added for chat media uploads
import { getMessaging } from "firebase/messaging"; // ← NEW: for push notifications
...
const messaging = getMessaging(firebaseApp); // ← NEW: create messaging instance
...
// Fields only admin can write — strip these before any user-facing save
...
const saveUser = async (uid: string, data: any) => {
  // Strip strictly admin-only fields, then update (not set) to avoid overwriting
  // fields we didn't intend to touch
  const safe: Record<string, any> = {};
  Object.entries(data).forEach(([k, v]) => {
    if (!ADMIN_ONLY_FIELDS.includes(k)) safe[k] = v;
  });
  await update(ref(db, `mk2_users/${uid}`), safe);
};
...
export {
  ...
  analytics,
  firebaseApp,
  messaging, // ← NEW: export messaging for push notifications
  fetchUser,
  saveUser,
  logEvent,
  fetchCollection,
