...
import { LeaderboardManager } from "./LeaderBoardManager";          // <-- ADDED
import { PackagesManager } from "./Packagesmanager"; // adjust path if needed
...
// ── Admin Calendar (for class scheduling) ────────────────────────────────────
    ...
    >
      ...
      >
        {cells.map((day, i) => {
          ...
          const isSel = selectedDate.toISOString().split("T")[0] === key;
...
function ClassesManager({ toast }: any) {
  ...
  const save = async () => {
    ...
    const data = {
      ...
      specificDate:
        scheduleMode === "date" ? calDate.toISOString().split("T")[0] : "",
      day: scheduleMode === "day" ? form.day : getDayNameFromDate(calDate),
    };
    if (editing) {
  ...
  const getDayNameFromDate = (d: Date) => {
    const map = [6, 0, 1, 2, 3, 4, 5];
    return DAYS[map[d.getDay()]];
  };
  ...
  const selectedDateKey = calDate.toISOString().split("T")[0];
  const selectedDayName = getDayNameFromDate(calDate);

  const classesOnDate = classes
    .filter((cls) => {
      if (cls.scheduleType === "date")
        return cls.specificDate === selectedDateKey;
          ...
          >
            ...
            <div style={{ marginBottom: 14 }}>
              ...
              >
                ...
                (Comp/Scaled/Beg). Members will see this formatted like a class
                card.
              </div>
              <textarea
                style={{
        ...
        <>
          ...
          <div style={{ marginBottom: 12 }}>
            ...
            {loading ? (
              ...
              >
                No classes on this date. Click "+ Add Class" to schedule one.
              </div>
            ) : (
...
const TABS = [
  ...
  { id: "classes", label: "📅 Classes", desc: "Schedule & calendar" },
  ...
  { id: "instagram", label: "📱 Instagram", desc: "Auto-sync Instagram posts" },
  {
    id: "packages",
    label: "🎟 Packages",
    desc: "Manage class credit packages",
  },
  // <-- NEW LEADERBOARD TAB
  { id: "leaderboard", label: "🏆 Leaderboard", desc: "Manage leaderboard entries" },
];
    ...
    >
      ...
      >
        ...
        >
          ...
          {tab === "leaderboard" && <LeaderboardManager toast={toast} />}
        </motion.div>
      </div>
...
// import { db } from "@/lib/firebase";
// import { motion, AnimatePresence } from "framer-motion";
// import { PackagesManager } from "./PackagesManager"; // adjust path
// const ADMIN_PASSWORD = "MK2R@2026";
// const DAYS = [
//   "Monday",
//   "Tuesday",
...
//   // ── Build classDateCounts for calendar dots ───────────────────────────────
//   const today = new Date();
//   today.setHours(0, 0, 0, 0);
//   const classDateCounts: Record<string, number> = {};
...
//   // ── Classes for selected calendar date ────────────────────────────────────
//   const selectedDateKey = calDate.toISOString().split("T")[0];
//   const selectedDayName = getDayNameFromDate(calDate);
...
//             {/* WOD Field – added after details textarea */}
//             <div style={{ marginBottom: 14 }}>
//               <label style={lbl}>WOD / Workout Detail (optional)</label>
//               <div
//                 style={{
