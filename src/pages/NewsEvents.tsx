...
// Colour map for admin-added news items
const TYPE_COLORS: Record<string, string> = {
  News: "hsl(217 91% 53%)",
  Event: "hsl(20 100% 50%)",
  Announcement: "hsl(38 92% 44%)",
};
    ...
    const unsub = onValue(newsRef, (snap) => {
      if (snap.exists()) {
        ...
        const list = Object.entries(data).map(([id, val]: [string, any]) => ({
          ...
          color: val.color || TYPE_COLORS[val.type] || "hsl(20 100% 50%)",
        }));
        // Sort newest first by date string (simple alphabetical works for "D Mon YYYY" format)
        list.reverse();
        setAdminNews(list);
      } else {
        setAdminNews([]);
        ...
        <div className="grid grid-cols-[repeat(auto-fill,minmax(280px,1fr))] gap-3.5">
          {filtered.map((n, i) => (
            ...
            >
              ...
              <div
                className="text-[11px] font-bold tracking-[0.06em] mb-2"
                style={{ color: n.color }}
              >
                {n.date}
              </div>
              <div className="text-sm text-muted-foreground leading-relaxed mb-3.5">
                {n.desc}
              </div>
              <button
                className="bg-transparent border rounded-md px-3.5 py-1.5 text-[11px] font-bold font-body cursor-pointer uppercase tracking-wide hover:bg-primary/5 transition-colors"
